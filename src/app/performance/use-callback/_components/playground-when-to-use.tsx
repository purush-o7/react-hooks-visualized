...
const questions = [
  ...
  {
    id: "dep",
    question: "Is this function in a dependency array?",
...
const code = `// USE useCallback when:
const handler = useCallback(() => { /* ... */ }, [dep]);
<MemoizedExpensiveChild onClick={handler} />

// SKIP useCallback when:
// - Child isn't wrapped in memo()
// - Function isn't used in deps arrays
// - The re-render is cheap anyway`;

export function PlaygroundWhenToUse() {
  ...
  function getVerdict() {
    ...
    if (yesCount >= 2) return { text: "useCallback helps here!", color: "green" };
    if (yesCount === 1) return { text: "Marginal benefit — use your judgement", color: "yellow" };
    return { text: "Don't bother — no benefit", color: "red" };
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <CodeBlock code={code} filename="when-to-use.tsx" />
      </CardContent>
    </Card>
  );
}
