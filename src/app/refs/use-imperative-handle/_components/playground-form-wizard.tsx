...
const FormStep = forwardRef<FormStepHandle, { fields: { name: string; label: string; placeholder: string }[] }>(
...
const code = `const FormStep = forwardRef(({ fields }, ref) => {
...
export function PlaygroundFormWizard() {
  ...
  const allDone = completedData.length === STEPS.length && isLastStep && completedData[currentStep];
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border p-4">
          <h3 className="text-sm font-medium mb-3">{STEPS[currentStep].title}</h3>
          <FormStep key={currentStep} ref={stepRef} fields={STEPS[currentStep].fields} />
        </div>
        ...
        {allDone && (
          <div className="rounded-lg bg-green-500/10 p-3">
            <p className="text-sm font-medium text-green-600 mb-2">All steps completed!</p>
            <div className="space-y-1">
              {completedData.map((data, i) => (
                <div key={i} className="text-xs font-mono text-muted-foreground">
                  Step {i + 1}: {JSON.stringify(data)}
                </div>
              ))}
            </div>
          </div>
        ...
        <p className="text-xs text-muted-foreground">
          The wizard parent calls stepRef.current.validate() before advancing.
          Each FormStep manages its own state — the parent only sees the
          imperative API.
        </p>

        <CodeBlock code={code} filename="form-wizard.tsx" />
      </CardContent>
    </Card>
  );
}
